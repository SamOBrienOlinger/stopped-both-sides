...
export function renderScene(state,options={}){
 ...
 const names=roles.map(role=>`<span data-role-tone="${role}"><strong data-no-translate>${esc(characterFor(role,state.cast[role]).name)}</strong><span>${roleLabels[role]}</span></span>`).join('');
 return `<figure class="encounter-scene ${compact?'encounter-scene--compact':''}" data-scene-id="${esc(state.id+'/'+m.nodeId)}" data-scene-phase="${m.phase}" data-scene-mode="${m.mode}"><div class="scene-stage ${m.separate?'scene-stage--separate':''}" role="img" aria-label="${esc(m.stage.title)}">${background(m.place,m.separate?'scene-location--public':'')}${m.separate?background(m.otherPlace,'scene-location--garda'):''}${actors}${prop}${m.separate?'<span class="scene-divider" aria-hidden="true"></span>':''}<span class="scene-setting">${esc(m.label)}</span></div><figcaption><div class="scene-cast-names">${names}</div><p class="scene-moment"><span>${m.choice?'Your chosen response':'The scene'}</span><span>${esc(m.choice?.text||m.stage.title)}</span></p></figcaption></figure>`;
}
