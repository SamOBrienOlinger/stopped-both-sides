...
export function createEncounterMode({siteRole,main,rerender,getOriginalSession,resetOriginalSession,announce=()=>{}}){
 ...
 function landing(){
  ...
  const galleries=roles.map(role=>{
   const gallery=characters[role].map(c=>`<button class="landing-character" data-action="paired-landing-character" data-role="${role}" data-character="${c.id}" aria-label="Choose character: ${c.name}" title="${c.name}">${characterImage(role,c.id)}<span class="landing-avatar">${characterImage(role,c.id)}</span><span class="landing-character-name" data-no-translate>${esc(c.name)}</span></button>`).join('');
   return `<div class="perspective-entry" data-role-tone="${role}"><p>${role==='public'?'You are stopped. Learn what you can ask and what you may need to do.':'You make the stop. Check your reasons, assumptions and responsibilities.'}</p><h3 class="cast-heading"><span>${role==='public'?'Public perspective':'Garda perspective'}</span> · <span>Choose your character</span></h3><div class="landing-cast" role="group" aria-label="${role==='public'?'Public perspective':'Garda perspective'}">${gallery}</div></div>`;
  }).join('');
  return `${resumeLink()}<section class="paired-entry paired-entry-visual reference-landing" aria-labelledby="paired-title"><div class="landing-instructions"><h2 id="paired-title">Which side will you try first?</h2><p class="landing-purpose">Know your rights. Understand fair decisions.</p><p class="small-note">Read a situation, choose a response, then find out why it matters.</p></div><div class="perspective-heroes overlapping-heroes">${heroes}</div><div class="perspective-casts">${galleries}</div></section><section class="how-to-play" aria-labelledby="how-to-title"><h2 id="how-to-title">How it works</h2><ol><li><strong>Read a situation</strong><span>A short, fictional encounter with Gardaí.</span></li><li><strong>Choose a response</strong><span>Try what you would say or do.</span></li><li><strong>Learn why it matters</strong><span>Get an explanation, then try the other side.</span></li></ol><p class="small-note">No sign-up. No timer. You can try again.</p></section>`;
 }
 function playHelp(){
 ...
 return {handles,view,introduction,landing,legacyBar,legacyScene:(original,index)=>{const s=legacyState(original);return renderScene(s,{step:s.history[index],compact:true});},updateSiteLink,handleClick,transferOriginal:(original,focus=true)=>navigate(legacyState(original),true,focus)};
}
