...
export function createEncounterMode({siteRole,main,rerender,getOriginalSession,resetOriginalSession,announce=()=>{}}){
 ...
 function landing(){
  const heroes=roles.map(role=>{
   ...
   return `<div class="perspective-hero" data-role-tone="${role}"><img src="${image}" srcset="${srcset}" sizes="(min-width: 1280px) 660px, 55vw" width="1672" height="941" alt="" decoding="async"><a class="primary perspective-hero-button" href="#encounters/${role}">${role==='public'?'Public perspective':'Garda perspective'}</a></div>`;
  }).join('');
  const cloud=new URL('../assets/brand/brick-cloud.webp',import.meta.url).href;
  const cloudSmall=new URL('../assets/brand/brick-cloud-480.webp',import.meta.url).href;
