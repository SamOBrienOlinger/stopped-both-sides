...
- Native buttons, links, select elements, checkboxes, disclosures and a modal dialog support keyboard interaction. The dialog uses native focus containment and Escape behaviour, with focus returned to the opener or stable header control.
...
Manual browser zoom/reflow, text-spacing override checks, keyboard and native dialog behaviour, VoiceOver, TalkBack, NVDA/JAWS, forced colours, touch targets and testing with disabled users remain necessary before asserting conformance. Further languages should receive qualified translation and review before publication. This change does not constitute legal, operational, language or independent accessibility sign-off.
