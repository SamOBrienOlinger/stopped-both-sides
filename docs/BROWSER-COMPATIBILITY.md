...
- Hero perspective links stay centred near the top of their images on desktop, with at least 48px height. On small screens they share an independent row above the images, keeping faces visible when labels grow. The images stay in one horizontal row at every tested width. Their inner edges overlap beneath a transparent LEGO-brick mosaic cloud. The character galleries reflow separately. Browser checks measure scene overlap, cloud centring, control alignment, containment and image loading in English and enlarged Irish text.
