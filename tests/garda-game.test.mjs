...
test('Revisiting a decision discards the later branch and rebuilds the recap',()=>{
 let state=advance(choose(startSession('street'),0));assert.equal(state.nodeId,'street-information');
 state=advance(choose(state,0));assert.equal(state.nodeId,'street-identity');
 state=previous(state);state=previous(state);assert.equal(state.nodeId,'street-start');assert.equal(state.history.length,0);
 state=advance(choose(state,1));assert.equal(state.nodeId,'street-enquiry');assert.equal(recap(state).length,1);
 assert.equal(recap(state)[0].choice.text,scenarios[0].nodes['street-start'].choices[1].text);
});
test('Invalid actions, replay and independent sessions are handled',()=>{
 const original=startSession('search');assert.equal(advance(original),original);assert.equal(choose(original,-1),original);assert.equal(choose(original,9),original);assert.equal(choose(original,NaN),original);assert.equal(previous(original),original);const selected=choose(original,1);assert.equal(original.selected,null);assert.equal(selected.selected,1);assert.deepEqual(startSession('search'),original);assert.throws(()=>startSession('missing'));assert.throws(()=>startSession('__proto__'));assert.throws(()=>startSession('constructor'));
});
